...
# --- HELPER: PARSE MARKDOWN TABLE TO DATAFRAME ---
    ...
    try:
        ...
        for line in lines:
            ...
            if capture_table and not table_finished:
                ...
                # Detect Table End (blank line after having captured rows)
                elif stripped == "" and len(table_lines) > 0:
                    table_finished = True
                # If we hit another header or text while capturing, table is done
                ...
                # Text between header and table start (e.g. "In USD mn")
                pre_table_text += "\n" + line
...
def create_excel(df):
    """Converts the parsed DataFrame to formatted Excel."""
    if df is None: return None
    try:
        import xlsxwriter # Lazy import to avoid crash if missing
    except ImportError:
        return None

    try:
        # CLEAN DATA (String -> Number)
        df_clean = df.copy()
        ...
        for col in df_clean.columns[1:]:
            df_clean[col] = df_clean[col].apply(clean_financial_num)

        # WRITE TO EXCEL
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            df_clean.to_excel(writer, sheet_name='Financial Summary', index=False)
            ...
            worksheet.set_column(1, len(df_clean.columns)-1, 15)
            ...
            for i, row in df_clean.iterrows():
                row_idx = i + 1
                item_name = str(row[df_clean.columns[0]]).lower()
                worksheet.write(row_idx, 0, row[df_clean.columns[0]], item_fmt)
...
def generate_company_report(ticker):
    ...
    1.  **Search Strategy (CRITICAL):**
        ...
        -   **Year Verification:** Before extracting ANY number, verify the **Fiscal Year End**.
    ...
    4.  **Rationale (Internal):** AFTER the main report, output a section header called `Appendix`. INCLUDE IT IN THE NEXT PAGE. In this section, detail your thought process.
        -   **Financial Data Audit:** For every year (FY23, FY24, LTM) in the Financial Summary, you must state:
            * **Exact Document Name:** (e.g., "Ford 2024 10-K")
            * **Page Number/Table Name:**
            * **Raw Figure Used:**
            * **Reasoning:** Explain why you assigned it to that specific column.
        -   **This is to prevent year-shifting errors.**

    5.  **Transparency & Footnotes:**
        -   **After EVERY section** include a small footnote starting with "*Source:*".
        -   **Financial Summary Specificity:** The footnote directly below the Financial Summary table MUST clarify if the figures are from **Audited Financial Statements**, an **Earnings Release**, or **Management Accounts**.
    
    6. **Data Freshness:** Use ONLY the most recent data available.
    ...
    *Source: Latest Rating Action Commentaries (Oct 2025).*
    ...
    *Source: Figures for FY23/24 derived from Audited Financial Statements; LTM figures derived from Q3 2025 Earnings Release.*
...
if st.session_state["report_text"]:
    ...
    if len(parts) > 1:
        ...
        rationale_text = parts[1].strip() # Keep just the text for searching
    else:
    ...
    # 2. PARSE FINANCIAL SUMMARY TABLE
    ...
    if df_financials is not None:
        ...
        # --- INTERACTIVE CROSS-QUERY TABLE ---
        st.subheader("Interactive Financial Summary")
        st.info("👆 **Click on any row** to see the Audit Trail (Source Document & Logic) for that item.")
        
        # Display Dataframe with Selection Mode
        ...
        if len(selection.selection.rows) > 0:
            ...
            # Search Appendix for this Item
            st.markdown(f"### 🔍 Audit Trail for: **{selected_item}**")
            
            # Simple keyword search in the Rationale Text
            relevant_lines = []
            rationale_lines = rationale_text.split('\n')
            found = False
            
            # Clean item name for search (remove ** and whitespace)
        ...
        # Fallback if parsing fails: Just show the whole markdown
    ...
    # Excel Button
    xls_data = create_excel(df_financials) if df_financials is not None else None
    with dl_col2:
        if xls_data:
            st.download_button(
            ...
            if df_financials is None:
                st.info("⚠️ Financial table structure not found.")
            else:
                 st.error("⚠️ 'xlsxwriter' module missing. Please install it.")
